...
class pyduino():
    ...
    def __analogJoystick__(self, sw_pin, Xpin, Ypin):
        '''
    ...
    def __waterlevel__(self, pin):
        '''
        This function gives you the value of a water level sensor
        Argument: analog pin number
        '''
        try:
            board = Arduino(self.port)
            ...
            pin = str(pin)
            HistoryValue = 0
            adc_id = board.get_pin('a:'+pin+':i')
            value = adc_id.read()
            if value is not None: 
                if ((HistoryValue>=value) and ((HistoryValue - value) > 10)) or ((HistoryValue<value) and ((value - HistoryValue) > 10)):
